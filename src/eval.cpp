...
void Env::define_let(const std::string& name, Value v) { if (scopes.empty()) push(); scopes.back()[name] = VarCell{std::move(v), false}; }
void Env::define_var(const std::string& name, Value v) { if (scopes.empty()) push(); scopes.back()[name] = VarCell{std::move(v), true}; }
void Env::assign(const std::string& name, Value v) {
  for (auto it = scopes.rbegin(); it != scopes.rend(); ++it) {
    auto f = it->find(name);
    if (f != it->end()) {
      if (!f->second.mut) throw std::runtime_error("runtime error: cannot assign to immutable 'let " + name + "'");
      f->second.val = std::move(v); return;
    }
  }
...
const FnDecl* Env::get_fn(const std::string& name) const { auto it=fns.find(name); return it==fns.end()?nullptr:it->second; }
...
static std::string to_string_value(const Value& v) {
  ...
  std::string s = "[";
  for (size_t i=0;i<arr.items.size();++i){ if(i) s += ", "; s += to_string_value(arr.items[i]); }
  s += "]";
  return s;
}
...
static bool equal_values(const Value& a, const Value& b) {
  ...
  for (size_t i=0;i<A->items.size();++i) if (!equal_values(A->items[i], B->items[i])) return false;
  return true;
}
...
static Value eval_variable(const Variable& v, const Env& env){
  Value out; if (!env.get(v.name, out)) throw std::runtime_error("runtime error: undefined variable '" + v.name + "'");
  return out;
}
...
std::optional<Value> exec_stmt(const Stmt& s, Env& env, bool* returned, Value* ret_val){
  ...
  return std::visit([&](auto const& node) -> std::optional<Value> {
    ...
    if constexpr (std::is_same_v<T, Let>) {
      Value v = eval_node(*node.init, env); env.define_let(node.name, std::move(v)); return std::nullopt;
    } else if constexpr (std::is_same_v<T, Var>) {
      Value v = eval_node(*node.init, env); env.define_var(node.name, std::move(v)); return std::nullopt;
    } else if constexpr (std::is_same_v<T, Assign>) {
      Value v = eval_node(*node.value, env); env.assign(node.name, std::move(v)); return std::nullopt;
    } else if constexpr (std::is_same_v<T, ExprStmt>) {
      return eval_node(*node.expr, env);
    } else if constexpr (std::is_same_v<T, Print>) {
      Value v = eval_node(*node.expr, env); std::cout << to_string_value(v) << "\n"; return std::nullopt;
    } else if constexpr (std::is_same_v<T, Block>) {
      env.push(); std::optional<Value> last;
      ...
      while (!node.cond || truthy(eval_node(*node.cond, env))) {
        ...
        last = exec_stmt(*node.body, env, &ret, &rv);
        if (ret) { env.pop(); mark_return(std::move(rv)); return std::nullopt; }
        if (node.step) (void)eval_node(*node.step, env);
      }
      env.pop();
      return last;
      ...
      env.define_fn(&node); return std::nullopt;
    } else if constexpr (std::is_same_v<T, Return>) {
      Value v = eval_node(*node.value, env); mark_return(std::move(v)); return std::nullopt;
    } else {
