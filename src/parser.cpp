...
Program Parser::parse_program() {
  Program p; while (!check(TokenKind::End)) p.push_back(statement()); return p;
}
StmtPtr Parser::parse_one_stmt() {
  auto s = statement(); if (!check(TokenKind::End)) throw std::runtime_error(pos_str(filename, current) + "parse error: expected end of input"); return s;
}
...
StmtPtr Parser::statement() {
  ...
  if (check(TokenKind::KwFor))    return for_stmt();       // for-in or C-style
...
StmtPtr Parser::if_stmt() {
  ...
  expect(TokenKind::LParen, "'('"); auto c = expression(); expect(TokenKind::RParen, "')'");
  auto t = statement();
...
// 'for' '(' ... ')'  -> C-style    |    'for' IDENT 'in' expr stmt  -> for-in
StmtPtr Parser::for_stmt() {
  ...
  if (check(TokenKind::LParen)) {
    ...
    if (!check(TokenKind::Semicolon)) {
      if (check(TokenKind::KwLet)) init = let_stmt();
      else if (check(TokenKind::KwVar)) init = var_stmt();
      else init = assign_or_expr_stmt();
    }
    expect(TokenKind::Semicolon, "';'");
    ExprPtr cond; if (!check(TokenKind::Semicolon)) cond = expression();
    expect(TokenKind::Semicolon, "';'");
    ExprPtr step; if (!check(TokenKind::RParen)) step = expression();
    expect(TokenKind::RParen, "')'");
    auto body = statement();
    return Stmt::make_for_c(std::move(init), std::move(cond), std::move(step), std::move(body));
  }
  // for-in (no parentheses): for ident in expr stmt
...
ExprPtr Parser::or_expr() {
  auto left = and_expr();
  while (check(TokenKind::OrOr)) { advance(); auto right = and_expr(); left = Expr::make_binary(std::move(left), BinaryOp::LOr, std::move(right)); }
  return left;
}
ExprPtr Parser::and_expr() {
  auto left = equality();
  while (check(TokenKind::AndAnd)) { advance(); auto right = equality(); left = Expr::make_binary(std::move(left), BinaryOp::LAnd, std::move(right)); }
  return left;
}
...
ExprPtr Parser::comparison() {
  ...
  while (check(TokenKind::Less) || check(TokenKind::LessEqual) || check(TokenKind::Greater) || check(TokenKind::GreaterEqual)) {
    ...
    auto right = term();
    BinaryOp bop;
    switch (op.kind) { case TokenKind::Less: bop=BinaryOp::Lt; break; case TokenKind::LessEqual: bop=BinaryOp::Le; break; case TokenKind::Greater: bop=BinaryOp::Gt; break; default: bop=BinaryOp::Ge; }
    left = Expr::make_binary(std::move(left), bop, std::move(right));
  }
  return left;
}
...
std::vector<ExprPtr> Parser::arg_list() {
  std::vector<ExprPtr> args; args.push_back(expression());
